...
If the provider serves a cert signed by a private CA that the runtime's trust store doesn't recognize (symptoms: `self signed certificate in certificate chain`, `unable to verify the first certificate`, or `invalid peer certificate: UnknownIssuer` under oam), the best fix is to trust that CA: point `NODE_EXTRA_CA_CERTS` at its PEM file. Node and oam 0.15.3+ both read it, and the connection stays fully verified:
...
This disables certificate-chain verification only - the connection is still TLS-encrypted end-to-end, but no longer authenticated, so prefer `NODE_EXTRA_CA_CERTS` wherever you can.
