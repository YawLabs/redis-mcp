...
All env vars are read from the MCP server's environment. The last three are read by the `redis-mcp` launcher rather than the server, so they have no effect when a host runs `dist/index.js` directly - and neither does the launcher's oam version check (see [Runtime](#runtime)):
...
| `REDIS_TLS_REJECT_UNAUTHORIZED` | unset | Set to `false` to skip TLS certificate verification - the chain and the hostname check. The connection is still encrypted, but no longer authenticated. A last resort for a private CA: prefer `NODE_EXTRA_CA_CERTS`, see [Connecting to managed Redis](#connecting-to-managed-redis-upstash-elasticache-redis-cloud-etc). |
| `REDIS_MCP_RUNTIME` | `auto` | Which JS runtime executes the server: `auto` (the newest [oam](https://oamjs.org) it can find at 0.15.3 or newer, else Node), `oam` (the same, but exit with an error instead of falling back), `node` (always Node - in-process under `npx`, handed off to Node on `PATH` when a client launches the command with `oam run`). Case-insensitive; any other value - a typo like `nodejs`, or one with surrounding spaces - behaves as `auto` without a warning. See [Runtime](#runtime). |
| `OAM_BIN` | unset | Path to an `oam` binary to use in preference to discovery, when it is 0.15.3 or newer. If it does not exist, is older, or will not run, the launcher says so on stderr and carries on with discovery. Ignored under `REDIS_MCP_RUNTIME=node`, and when the launcher is already running under oam 0.15.3+ unless `REDIS_MCP_SANDBOX=1`. |
| `REDIS_MCP_SANDBOX` | unset | Exactly `1` runs the server under oam's `--permission` sandbox: filesystem and child processes denied, network limited to the host and port in `REDIS_URL` (port `6379` if the URL names none). An IPv6 literal (`redis://[::1]:6379`) is pinned like any other host. The host and port are read the way ioredis reads them, so a scheme-less `REDIS_URL` (`127.0.0.1:6379`), a bare port number, a `host` or `port` in the query string, and a URL with no host (`localhost`) are pinned too. If `REDIS_URL` names no single TCP endpoint the grant can spell - a unix socket path (including a `path` in the query string), a URL that does not parse, an empty host, a host containing a comma, or a port outside 1-65535 - the network grant is left open, and the launcher says so on stderr. A `REDIS_MCP_SANDBOX` value other than `1` is ignored without a warning. It has no effect unless a fresh oam is actually spawned. Every fallback that runs the server without it says so on stderr, but a host may not show stderr, so pair it with `REDIS_MCP_RUNTIME=oam`, which exits instead of falling back. The launcher only spawns oam 0.15.3 or newer, which has every fix the sandbox depends on: per oam's changelog, `--permission` did not cover all of `fs` and `child_process` until 0.9.1, and the port grant was not exact until 0.15.0. |

### Connecting to managed Redis (Upstash, ElastiCache, Redis Cloud, etc.)

Use a `rediss://` URL for TLS. It works the same on Node and on oam, sandbox included.

If the provider serves a cert signed by a private CA that the runtime's trust store doesn't recognize (symptoms on stderr: `self-signed certificate in certificate chain`, `unable to verify the first certificate`, or `self-signed certificate`, the same on Node and oam), the best fix is to trust that CA: point `NODE_EXTRA_CA_CERTS` at its PEM file. Node and oam 0.15.3+ both read it, and the connection stays fully verified:

```json
"env": {
  "REDIS_URL": "rediss://default:pass@host:6379",
  "NODE_EXTRA_CA_CERTS": "/path/to/provider-ca.pem"
}
```

If you cannot get the CA certificate, add `REDIS_TLS_REJECT_UNAUTHORIZED=false` instead:
...
This turns certificate verification off entirely - the chain and the hostname check - so the connection is still TLS-encrypted end-to-end, but no longer authenticated. Prefer `NODE_EXTRA_CA_CERTS` wherever you can.

### Runtime

The published `redis-mcp` command is a small launcher that prefers the newest [oam](https://oamjs.org) JavaScript runtime it can find and falls back to Node. It only uses the latest oam release, currently **0.15.3**, and never serves on an older one. **If you do not have oam, nothing changes:** the fallback is automatic, and because npm has already started Node to run the launcher, it is an in-process `import()` of the server with no extra spawn.

**Why 0.15.3.** Before it, oam's TLS socket lacked some of the `net.Socket` methods ioredis calls and did not close itself when the server hung up, so `rediss://` crashed the server on its first tool call ([YawLabs/oam#132](https://github.com/YawLabs/oam/issues/132)), and oam ignored `NODE_EXTRA_CA_CERTS` ([YawLabs/oam#136](https://github.com/YawLabs/oam/issues/136)). 0.15.3 fixes both, so on it TLS behaves as it does on Node. With only an older oam available, the launcher runs the server on Node instead - in its own process under `npx`, or handed off to Node on `PATH` when a host launches it with an old `oam run` - and says to update oam from [oamjs.org](https://oamjs.org). It exits instead when that old `oam run` host has no Node on `PATH`, or under `REDIS_MCP_RUNTIME=oam`. A host that bundles its own oam needs the host's update. A host that runs `dist/index.js` directly under oam skips all of this; on an oam below 0.15.3 the server then answers every `rediss://` tool call with an error naming the version to update to, rather than crashing.

**How the runtime is chosen:**

- **Already on oam.** If the launcher itself is running under oam 0.15.3+ (a host that runs `oam run` on the `bin`), the server is imported into that process: nothing is discovered or spawned, `OAM_BIN` is ignored, and `REDIS_MCP_RUNTIME=oam` is satisfied. `REDIS_MCP_SANDBOX=1` skips this step, because only a freshly spawned oam can apply `--permission`: the launcher relaunches the oam it is running on with the sandbox, even when that oam is in no install location and not on `PATH` (a usable `OAM_BIN` still takes precedence).
- **Otherwise, discovery.** The launcher uses `OAM_BIN` if it is 0.15.3 or newer. Otherwise it asks every oam it can find - the default install locations (`%LOCALAPPDATA%\oam\bin` then `~/.oam/bin` on Windows, `~/.oam/bin` elsewhere), then `PATH` - for its version, and spawns the newest one at 0.15.3 or newer; on a tie the installed copy wins. On Windows only `oam.exe` counts. An `OAM_BIN` that does not exist, is older, or will not run is named on stderr and discovery carries on.
- **No usable oam.** Under `auto` the server runs in the launcher's own process instead (an old oam host hands off, see below). The launcher says so on stderr when it passed over an `OAM_BIN`, a found oam that was too old or would not run, or (on Windows) an `oam.cmd` / `oam.bat` on `PATH`, when the chosen oam failed to launch, and whenever `REDIS_MCP_SANDBOX=1` was set. A note that passed over a working oam that is only too old says to update it. With no oam found at all and no sandbox asked for, it falls back silently; the handoff from an old oam host is always noted, in one message written once the launcher knows whether a Node was found. Once oam has started, a failure while running the server is a startup failure, not a fallback.
- **An old oam host never serves.** If the launcher is running under an oam older than 0.15.3, it hands the server off to the newest usable oam, else to Node on `PATH`, and exits with an error when there is neither.
...
No fallback carries `--permission` - not in-process, and not a handoff to Node - so with `REDIS_MCP_SANDBOX=1` under `auto` the server can end up **unsandboxed**. Every such fallback says so on stderr (`without --permission`), but a host may not show stderr. Set `REDIS_MCP_RUNTIME=oam` alongside it when an unsandboxed server is not acceptable.
