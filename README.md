...
- **No usable oam.** Under `auto` the server runs in the launcher's own process instead. The launcher says so on stderr when it passed over an `OAM_BIN`, a found oam that was too old or would not run, or (on Windows) an `oam.cmd` / `oam.bat` on `PATH`, or when the chosen oam failed to launch; a machine with no oam at all falls back silently. Once oam has started, a failure while running the server is a startup failure, not a fallback.
...
No fallback carries `--permission` - not in-process, and not a handoff to Node - so with `REDIS_MCP_SANDBOX=1` under `auto` the server can end up **unsandboxed**, without a message when no oam was found at all. Set `REDIS_MCP_RUNTIME=oam` alongside it when that is not acceptable.
