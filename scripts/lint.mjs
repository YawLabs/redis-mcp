 ...
 * directly, with no npm anywhere in the picture: 2.5.4 dies with exit 139,
 * while 2.4.16 and 2.5.13 check a source tree normally and return a real
 * pass/fail. The defect is per-version, not permanent -- nothing here should
 * be read as "arm64 biome is broken".
 *
 ...
 * formatting later. `release.sh` has a CI-mode branch, but no workflow exists
 * to run it; the workstation run is what publishes. Whatever this script
 * reports is the ONLY lint signal that exists before `release.sh` publishes to
 * npm.
 *
 ...
 * They were already apart in this repo when the flaw was found -- $schema
 * 2.4.12, lockfile 2.4.16 -- so the gate was checking the source with a version
 * the repo does not use. That gap is not cosmetic: on a sibling repo the same
 * skew ran 2.4.12 against a tree installing 2.5.4 and reported a clean pass
 * over three genuine lint errors the installed version rejects.
 *
...
function emulatedX64Binary(version) {
  ...
  console.error(`[lint] routing around the win32-arm64 biome build on this host; provisioning x64 ${version} under emulation`);
...
if (crashed) {
  ...
  console.error(
    `[lint] biome ${how} (${binary}).\n` +
      "Some Windows ARM64 biome builds crash exactly like this (2.5.4 measured; 2.4.16 and\n" +
      "2.5.13 do not), which is why this script routes that host through the x64 build by\n" +
      "default -- so check the YAWLABS_BIOME_BIN / YAWLABS_BIOME_NATIVE overrides.",
  );
  process.exit(1);
}
