...
### Fixed
- **Launcher fallbacks say when the sandbox was not applied, and never promise a fallback that does not happen.** Every fallback note under `REDIS_MCP_SANDBOX=1` now says `without --permission` — including the one with nothing else to report, which used to be silent. A note that passed over a working oam that is only too old says to update it. A host oam below the floor hands off in a single message written once the launcher knows whether Node is on `PATH`, instead of first announcing "using Node instead" and then exiting because there was none.
