...
- **`REDIS_MCP_SANDBOX=1` no longer denies every connection for an IPv6 literal `REDIS_URL`** ([#14](https://github.com/YawLabs/redis-mcp/issues/14)). The launcher built the network grant from WHATWG `URL#hostname`, which keeps the brackets on an IPv6 address, so `redis://[::1]:6391` became `--allow-net=[::1]:6391`. oam matches a socket grant as a whole string against the bare `host:port` it is checking — `::1:6391`, the address ioredis actually dials — so the grant never matched and every tool call came back `Access to this API has been restricted`. The grant is now derived the way ioredis's own URL parser reads `REDIS_URL`: brackets off, a scheme-less DSN such as `127.0.0.1:6379` read as if it began with `redis://` (it used to leave the network open), a bare port number read as `localhost`, and a `host` or `port` in the query string used when the URL itself names none (`redis://127.0.0.1?port=6391` used to be granted port 6379, which oam denied). A test compares the grant with the host and port a real ioredis client resolves, defaults included. Measured on oam 0.15.2: `--allow-net=::1:6391` admits `::1` port 6391 and still denies port 63910. When the grant still cannot be pinned — a `REDIS_URL` with no host, like `redis:///0`, or one nothing can parse — the launcher now says on stderr that the sandbox is leaving network access open, instead of opening it silently; a blank `REDIS_URL` stays quiet, since the server reports that itself. The note never repeats `REDIS_URL`, which can carry a password. The README's `REDIS_MCP_SANDBOX` row drops the "use a hostname" caveat and says the launcher reports an open grant.
