...
- **A numeric env value between `0` and `1` no longer disables the cap it was meant to set.** `REDIS_MAX_VALUE_BYTES`, `REDIS_MAX_KEYS` and `REDIS_SCAN_COUNT` rejected only values at or below `0` and then floored, so `0.5` passed the check and became `0`. A value cap of `0` sent `GETRANGE key 0 -1`, which Redis reads as the whole string, returned it whole and reported `truncated: true`; a key cap of `0` made every scan return no keys while reporting itself truncated with a live cursor; a `COUNT` of `0` is rejected by Redis with a syntax error, so every scan tool call failed. All three now treat anything below `1` like the other invalid inputs and fall back to the default. The README row for `REDIS_MAX_VALUE_BYTES` drops the "at least `1`" caveat.
