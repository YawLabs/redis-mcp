...
- **`REDIS_MCP_SANDBOX=1` no longer denies every connection for an IPv6 literal `REDIS_URL`.** The launcher built the network grant from WHATWG `URL#hostname`, which keeps the brackets on an IPv6 address, so `redis://[::1]:6391` became `--allow-net=[::1]:6391`. oam matches a socket grant as a whole string against the bare `host:port` it is checking — `::1:6391`, the address ioredis actually dials — so the grant never matched and every tool call came back `Access to this API has been restricted`. The brackets now come off, and a test checks that the grant's host and port are exactly the ones ioredis dials. Measured on oam 0.15.2: `--allow-net=::1:6391` admits `::1` port 6391 and still denies port 63910. And when the grant cannot be pinned at all — a `REDIS_URL` with no host, or one that does not parse as a URL but that ioredis accepts anyway, like `127.0.0.1:6379` — the launcher now says on stderr that the sandbox is leaving network access open, instead of opening it silently. The note never repeats `REDIS_URL`, which can carry a password.

### Documentation
- **The README no longer says TLS needs Node.** The remaining oam gap is that it does not read `NODE_EXTRA_CA_CERTS` ([YawLabs/oam#136](https://github.com/YawLabs/oam/issues/136)), so a private CA under oam needs `REDIS_TLS_REJECT_UNAUTHORIZED=false` or `REDIS_MCP_RUNTIME=node`; the sandbox row no longer excludes `rediss://`, and no longer tells IPv6 users to switch to a hostname.
