...
### Fixed
- **`REDIS_MCP_SANDBOX=1` now applies under `oam run` even when that oam cannot be found by discovery.** A host that bundles its own oam and runs the launcher with it keeps that binary out of the install locations and off `PATH`, so the launcher found no oam to spawn and served in-process, unsandboxed. It now relaunches the oam it is running on with `--permission`. A usable `OAM_BIN` still wins, and the host binary is relaunched only when it answers `--version` as that same oam.
- **Launcher fallbacks say when the sandbox was not applied, and never promise a fallback that does not happen.** Every fallback that runs the server under `REDIS_MCP_SANDBOX=1` now says `without --permission` on stderr — including the one with nothing else to report, which used to be silent, and the handoff to Node from an old oam host, on the line that names the Node. A note that passed over a working oam that is only too old says to update it. A host oam below the floor hands off in a single message written once the launcher knows whether Node is on `PATH`, instead of first announcing "using Node instead" and then exiting because there was none. With no Node there, the error no longer mentions the sandbox, and `REDIS_MCP_RUNTIME=node` on a current oam says it found no Node instead of printing `redis-mcp: , and no Node was found on PATH` and telling you to update an oam that is already up to date.
...
### Documentation
- **A private CA is now documented through `NODE_EXTRA_CA_CERTS` on both runtimes.** The README recommends trusting the provider's CA over disabling verification, quotes the certificate errors Node and oam actually print, says `REDIS_TLS_REJECT_UNAUTHORIZED=false` skips the hostname check as well as the chain, and no longer says `NODE_EXTRA_CA_CERTS` works only on Node. Tests verify the fixture's certificate through that variable alone, with verification on, on Node, under `oam run` and inside the sandbox, each against a control that fails on the certificate without it.

## [0.4.2] — 2026-09-14

### Fixed
- **`REDIS_MCP_SANDBOX=1` no longer denies every connection for an IPv6 literal `REDIS_URL`.** The launcher built the network grant from WHATWG `URL#hostname`, which keeps the brackets on an IPv6 address, so `redis://[::1]:6391` became `--allow-net=[::1]:6391`. oam matches a socket grant as a whole string against the bare `host:port` it is checking — `::1:6391`, the address ioredis actually dials — so the grant never matched and every tool call came back `Access to this API has been restricted` ([#14](https://github.com/YawLabs/redis-mcp/issues/14)). The host and port are now resolved the way ioredis resolves them: brackets off the URL's host, a scheme-less DSN such as `127.0.0.1:6379` read as if it began with `redis://` (it used to leave the network open), a bare port number read as that port on `localhost` — with the whitespace or sign ioredis tolerates — a `host` or `port` in the query string used when the URL itself names none (`redis://127.0.0.1?port=6391` used to be granted port 6379, which oam denied), a URL with no host read as `localhost`, and the port read with `parseInt`, so `06391` is `6391`. A test compares the grant with the host and port a real ioredis client resolves, defaults included, across 29 shapes, and checks that every shape ioredis dials as a unix socket gets no pinned grant. Measured on oam 0.15.2: `--allow-net=::1:6391` admits `::1` port 6391 and still denies port 63910. When `REDIS_URL` names no single TCP endpoint the grant can spell — a unix socket path, including a `path` in the query string; a URL that does not parse; an empty host; a host containing a comma, which oam would split into a grant for every port on the first host; or a port outside 1-65535, which Node refuses and oam 0.15.2 clamps (`70000` dials `65535`) — the launcher now says on stderr that the sandbox is leaving network access open, instead of opening it silently; a blank `REDIS_URL` stays quiet, since the server reports that itself. The note never repeats `REDIS_URL`, which can carry a password. The README's `REDIS_MCP_SANDBOX` row drops the "use a hostname" caveat and says the launcher reports an open grant.
...
- **A numeric env value between `0` and `1` no longer disables the cap it was meant to set.** `REDIS_MAX_VALUE_BYTES`, `REDIS_MAX_KEYS` and `REDIS_SCAN_COUNT` rejected only values at or below `0` and then floored, so `0.5` passed the check and became `0`. A value cap of `0` sent `GETRANGE key 0 -1`, which Redis reads as the whole string, returned it whole and reported `truncated: true`. A key cap of `0` sent `LRANGE key 0 -1` and `ZRANGE key 0 -1` from `redis_get` — the whole list or zset, with `truncated: false` — and made every scan return no keys, flagged `truncated` with a live cursor when the keyspace took more than one `SCAN` page and `truncated: false` when it did not. A `COUNT` of `0` is rejected by Redis with a syntax error, so every `redis_advisor` call and every `redis_scan` call without its own `count` failed. All three now treat anything below `1` like the other invalid inputs and fall back to the default. The README rows for all three say so, and the `REDIS_MAX_VALUE_BYTES` row drops the "at least `1`" caveat.
